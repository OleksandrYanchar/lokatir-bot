...
def create_user_markup() -> types.ReplyKeyboardMarkup:
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add("/quiz", "/creators", "/ban").add("/HowRomanAreYou", "/top", "/rofl").add("/sendFeedBack", "/help")
    return markup


def create_admin_markup() -> types.ReplyKeyboardMarkup:
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
...
def create_users_controll_markup() -> types.ReplyKeyboardMarkup:
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add("/sendAlert", "/sendMessage", "/addUser", "/users", "/back")
    return markup


def create_notes_n_stats_markup() -> types.ReplyKeyboardMarkup:
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add("/addNote", "/notes", "/stats", "/back")
    return markup


@dp.message_handler(commands=["admin", "userMenu", "TrackControll", "UsersControll", "NotesNStats", "back"])
async def admins_murkups(message: types.Message) -> None:
